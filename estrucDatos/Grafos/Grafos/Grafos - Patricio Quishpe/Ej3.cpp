//Ejercicio 3: Determinar si es un grafo vacío (solo posee vértices aislados).
...
int main(){
	...
	void verificarGrafo(GrafoMatriz g);
	...
	verificarGrafo(g);					//llamdo a la funcion que determina si el grafo es vacio
...
void verificarGrafo(GrafoMatriz g){
	if(verVertice(g)){
		cout<<"\nEl Grafo SI es vacio...\n";
	}else{
		cout<<"\nEl Grafo NO es vacio...\n";
	}
}
