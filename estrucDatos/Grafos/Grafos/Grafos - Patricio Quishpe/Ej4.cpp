//Ejercicio 4: Calcular el grafo transpuesto de un grafo original (posee iguales vértices, con las direcciones de los arcos invertidas).
...
int main(){
	...
	void grafoTranspuesto(GrafoMatriz *g);
	...
	grafoTranspuesto(&g);				//Llamado a la funcion para calcular el grafo transpuesto
...
void grafoTranspuesto(GrafoMatriz *g){
    ...
    //Se crea una copia temporal de la matriz de adyacencia
    int** tempMatAd = new pint[n];
    for (int i = 0; i < n; i++){
        tempMatAd[i] = new int[n];
        for (int j = 0; j < n; j++){
            tempMatAd[i][j] = g->getArco(i, j);
        }
    }
    //Se invierten las direcciones de los arcos en la matriz de adyacencia original
    for (int i = 0; i < n; i++){
        for (int j = 0; j < n; j++){
            g->setArco(i, j, tempMatAd[j][i]);
        }
    }
    //Eliminamos la matriz temporal
    for (int i = 0; i < n; i++){
        delete[] tempMatAd[i];
    }
    delete[] tempMatAd;
