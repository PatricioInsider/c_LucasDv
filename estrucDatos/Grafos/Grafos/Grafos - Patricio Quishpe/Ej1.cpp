//Ejercicio 1: Calcular el grado de cada vértice de un grafo (cantidad de arcos de los que el vértice es extremo)
...
int main(){
	...
	void gradoVertices(GrafoMatriz g);
...
GrafoMatriz ingresarVertices(int n1){				//funcion que ingresa los datos de los vÃ©rtices de un grafo
	TipoG aux;
	GrafoMatriz a(n1);
...
void gradoVertices(GrafoMatriz g){
   ...
   for (i = 0; i < g.getNumVerts(); i++){
        ...
        cout << endl << "-Vertice " << i+1 << ": " << g.getVertice(i).getDato() << ", grado: " << grado;
    }
}
